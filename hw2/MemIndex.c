...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  // make a key for the first work so you can look it up
  key = FNVHash64((unsigned char*) query[0], strlen(query[0]));

  // look it up
  if (HashTable_Find(index, key, &kv)) {
    // kv.value will have the matching documents
    wp = (WordPostings*)kv.value;

    // first allocate the linked list
    ret_list = LinkedList_Allocate();

    // wp->postings is a HT of all the documents that the key word is in
    // we need to iterate through all the keys of this HT to get each document
    HTIterator* itr = HTIterator_Allocate(wp->postings);
    while (HTIterator_IsValid(itr)) {
      // get the document
      HTIterator_Get(itr, &kv);

      // the document in in kv, so now malloc a SearchResult
      SearchResult* sr = (SearchResult*) malloc(sizeof(SearchResult));

      // in kv, the key is the docid and the value is
      // all of the places in the docid the word is
      sr->doc_id = kv.key;
      sr->rank = LinkedList_NumElements((LinkedList*)kv.value);

      // append sr to ret_list
  ...
  for (i = 1; i < query_len; i++) {
    ...
    // find the set of matched for the next query

    // Find the hash value of the next query.
    // go to index, get the vlaue of this word.
    // This will be a word positions struct.
    // in this struct, we have a char that is the word,
    // and then a hash table.
    // The keys of this hash table are the docIDs of all
    // the documents the word is in.
    // the values are linked list with the offsets of
    // the word in the documents.

    wp = (WordPostings*) kv.value;
    HashTable* postingsHT = wp->postings;
    ...
    for (j = 0; j < num_docs; j++) {
      SearchResult* sr1;
      LLIterator_Get(ll_it, (LLPayload_t*) &sr1);
      DocID_t di1 = sr1->doc_id;

      HTKeyValue_t kv1;
      if (HashTable_Find(postingsHT, di1, &kv1)) {
        // add to the rank of the ret_list
        // the amount to add is the lenght of LL which is kv1 value
        int add_length = LinkedList_NumElements((LinkedList*)kv1.value);
        sr1->rank += add_length;
