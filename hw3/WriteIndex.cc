...
>>>>>>> meganaHW4
  int mem_bytes = WriteMemIndex(f, mi, cur_pos);
  if (mem_bytes == kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }

  // otherwise we succesfully wrote the mem index
  cur_pos += mem_bytes;


  // STEP 2.
  // Finally, backtrack to write the index header and write it.
  int h_bytes = WriteHeader(f, dt_bytes, mem_bytes);
<<<<<<< HEAD
=======

  // check if header is 0 bytes than no file
...
>>>>>>> meganaHW4
  ...
  int num = fseek(f, sizeof(IndexFileHeader), SEEK_SET);
  if (num != 0) {
    return kFailedWrite;
  }
  // now we are at the end of the header
<<<<<<< HEAD
=======
  ...
  for (int i  = 0; i < doctable_bytes + memidx_bytes; i++) {
    ...
    char c;
    int res = fread(&c, sizeof(c), 1, f);
    if (res != 1) {
      perror("Error reading file");
      return kFailedWrite;  //TODO: ADDED MB MK
    }

    // feed c into the index file
    // NOTE: this definition is in Utils.h
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    // get the linked list at each bucket
    LinkedList* ll = ht->buckets[i];
    int num_elements = LinkedList_NumElements(ll);

    // new_bytes is the number of bytes written froms this ll
    // first thing to do for this is to add the bytes written
    // from the record to the new_bytes

    new_bytes = WriteHTBucketRecord(f, record_pos, num_elements, bucket_pos);
    if (new_bytes < 0) {
      return kFailedWrite;
    }
    record_pos += new_bytes;

    // then jump forward in the file to write the bucket contents itself.
    // corner case: if ll is empty, do nothing else
    // otherwise: call write record on the ll
    // the methods for these are right underneath
    if (num_elements != 0) {
      new_bytes = WriteHTBucket(f, bucket_pos, ll, fn);
      if (new_bytes < 0) {
        return kFailedWrite;
      }
      bucket_pos += new_bytes;
    }
  }
...
<<<<<<< HEAD
  // the methods for this are in layoutStructs.h
=======
  //the methods for this are in layoutStructs.h
>>>>>>> meganaHW4
...
=======
  if (fwrite(&br, sizeof(BucketRecord), 1, f) != 1) { // TODO MK
>>>>>>> meganaHW4
    return kFailedWrite;
  }
...
>>>>>>> meganaHW4
    ...
    int fn_bytes = fn(f, element_pos, kv);
    if (fn_bytes == kFailedWrite) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
<<<<<<< HEAD
...
>>>>>>> meganaHW4
    ...
    element_pos += fn_bytes;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
<<<<<<< HEAD
  int16_t file_name_bytes = strlen(reinterpret_cast <const char*>(kv->value));
=======
  int16_t file_name_bytes = strlen(reinterpret_cast <const char*>(kv->value));  // you may want to change this
>>>>>>> meganaHW4
...
<<<<<<< HEAD
  if (fwrite(reinterpret_cast <const char*>(kv->value),
      file_name_bytes, 1, f) == 0) {
=======
  if (fwrite(reinterpret_cast <const char*>(kv->value), file_name_bytes, 1, f) == 0) {
>>>>>>> meganaHW4
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    void* pos;
    LLIterator_Get(it, reinterpret_cast<void **>(&pos));
    DocPositionOffset_t offset = reinterpret_cast<intptr_t>(pos);
    position.position = static_cast<uint32_t>(offset);
<<<<<<< HEAD
=======
...
>>>>>>> meganaHW4
  ...
  return sizeof(DocIDElementHeader) + num_positions * sizeof(position);
}
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(header) + ht_bytes + word_bytes;  // you may want to change this
}
}  // namespace hw3
