...
// Takes in the query inputted by user and turns it into lowercase
// and split query into a vector of words. Return vector of
// words.
vector<string> GetQueryWords(string input);
// Takes in arguments of a vector of query words and a QueryProcessor.
// in order to process the query using the ProcessQuery method
// and prints the out the file path and ranking for each word and
// prints out [no result] if there are no file path and ranking for
// any of the words.
void QueryWordProcessor(const QueryProcessor& qp, const vector<string>& words);
...
<<<<<<< HEAD
  ...
  // now we have all the indexes and we need to put these into a
  // query processer
  ...
  while (1) {
    ...
    vector<string> words = GetQueryWords(input);
    QueryWordProcessor(qp, words);
=======
  ...
  while (1) { 
    ...
    // now we have the query words
    // we can now put them into the ProcessQuery method
    auto q_res = qp.ProcessQuery(words);

    // q_res is the list of all the documents for this query
    // if its empty, print no results
    if (q_res.size() == 0) {
...
void QueryWordProcessor(const QueryProcessor& qp, const vector<string>& words) {
  ...
  if (q_res.empty()) {
    ...
    for (const auto& qr : q_res) {
    cout << "  " << qr.document_name <<
            " (" << qr.rank << ")" << endl;
    }
  }
}
