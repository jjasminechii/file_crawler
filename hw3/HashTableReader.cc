...
=======
  ...
  fread(&header_.num_buckets, sizeof(header_), 1, file_);
>>>>>>> meganaHW4
...
<<<<<<< HEAD
  ...
  Verify333(fread(&bucket_rec.position, sizeof(bucket_rec.position),
            1, file_) == 1);
=======
...
>>>>>>> meganaHW4
  ...
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    ...
    fread(&epr.position, sizeof(epr.position), 1, file_);
    epr.ToHostFormat();
