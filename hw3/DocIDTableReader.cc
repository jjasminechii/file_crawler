...
=======
    ...
    fread(&curr_header.doc_id, sizeof(curr_header.doc_id), 1, file_);
    fread(&curr_header.num_positions, sizeof(curr_header.num_positions), 1, file_);
...
<<<<<<< HEAD
      ...
      for (int i = 0; i < curr_header.num_positions; i++) {
        DocIDElementPosition cur_pos;
        Verify333(fread(&cur_pos, sizeof(cur_pos), 1, file_) == 1);
        // to host
        cur_pos.ToHostFormat();
        // add to end of ret list
        ret_list.push_back(cur_pos.position);
=======
      std::list<DocPositionOffset_t> ret_list;
      for (int i = 0; i < curr_header.num_positions; i++) {
        DocPositionOffset_t cur_pos;
        fread(&cur_pos, sizeof(cur_pos), 1, file_);

        // to host
        cur_pos = ntohl(cur_pos);

        // add to end of ret list
        ret_list.push_back(cur_pos);
>>>>>>> meganaHW4
      }
...
<<<<<<< HEAD
    Verify333(fseek(file_,
              ...
              SEEK_SET) == 0);
=======
    fseek(file_, offset_ + sizeof(BucketListHeader) + i * sizeof(BucketRecord), SEEK_SET);
>>>>>>> meganaHW4
...
=======
    fread(&bucket_rec.chain_num_elements, sizeof(bucket_rec.chain_num_elements), 1, file_);
    fread(&bucket_rec.position, sizeof(bucket_rec.position), 1, file_);
>>>>>>> meganaHW4
...
<<<<<<< HEAD
      Verify333(fread(&element_pos.position, sizeof(element_pos.position),
                1, file_) == 1);
      element_pos.ToHostFormat();
      Verify333(fseek(file_, element_pos.position, SEEK_SET) == 0);
=======
      fread(&element_pos.position, sizeof(element_pos.position), 1, file_);
      element_pos.ToHostFormat();
...
=======
      fread(&element.doc_id, sizeof(element.doc_id), 1, file_);
      fread(&element.num_positions, sizeof(element.num_positions), 1, file_);
